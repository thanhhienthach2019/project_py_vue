// composables/settings/usePermissionPolicyRealtime.ts
...
export function usePermissionPolicyRealtime() {
  ...
  const off = onMessage(({ action, type, item }) => {
    ...
    const index = store.policies.findIndex(i => `${i.ptype}-${i.v0}-${i.v1}-${i.v2}` === key)

    switch (action) {
      case "create":
        if (index === -1) store.policies.push(item)
        break
      case "update":
        if (index !== -1) store.policies.splice(index, 1, item)
        break
      case "delete":
        if (index !== -1) store.policies.splice(index, 1)
        break
    }
  })
