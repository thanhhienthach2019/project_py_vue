<template>
    <transition name="fade">
      <div
        v-if="visible"
        :id="toastId"
        class="fixed bottom-4 right-4 z-50 flex items-center w-full max-w-xs p-4 mb-4 bg-white rounded-lg shadow-sm"
        role="alert"
      >
        <!-- Icon container -->
        <div
        ...
        <!-- Message -->
        <div class="ml-3 text-sm font-normal text-gray-500">{{ message }}</div>
        <!-- Close button -->
        <button
          type="button"
          @click="dismissToast"
          class="ml-auto -mr-1 -mt-1 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8"
          aria-label="Close"
        >
