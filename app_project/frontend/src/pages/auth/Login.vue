...
<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-900 to-purple-900 flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white/5 backdrop-blur-xl rounded-2xl shadow-xl p-8 space-y-8 transform transition-all duration-300 hover:shadow-2xl">
      <!-- Header Section -->
      <div class="text-center space-y-4">
        ...
        <p class="text-gray-300 text-sm">Welcome back! Please sign in to continue</p>
      </div>
      ...
      <form @submit.prevent="handleLogin" class="space-y-6">
        ...
        <div class="group relative">
          <div class="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-400 group-focus-within:text-indigo-400">
          ...
          <label 
            for="username" 
            class="absolute left-9 top-2 px-1 text-gray-400 text-sm transition-all pointer-events-none
                   group-focus-within:!-top-2.5 group-focus-within:!text-xs group-focus-within:text-indigo-400
                   [input:not(:placeholder-shown)+&]:-top-2.5 [input:not(:placeholder-shown)+&]:text-xs"
          >
            Username
          </label>
        </div>
        ...
        <button
          type="submit"
          :disabled="loading"
          class="w-full py-3 px-6 bg-gradient-to-r from-indigo-500 to-purple-500 text-white font-semibold rounded-lg
                 shadow-lg hover:shadow-indigo-500/20 transition-all duration-300 relative overflow-hidden"
        >
        ...
        <div class="grid grid-cols-2 gap-3">
          <button
            type="button"
            class="flex items-center justify-center gap-2 py-2.5 bg-white/5 rounded-lg border border-white/10
      ...
      <p class="text-center text-gray-400 text-sm">
        ...
        <a href="#" class="text-indigo-400 hover:text-indigo-300 transition-colors">Create account</a>
      </p>
    </div>
  </div>
</template>