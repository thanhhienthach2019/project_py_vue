    ...
    >
      <div class="flex items-center space-x-6">
        ...
        <div>
          ...
          <p class="text-teal-200 mt-2 font-medium">
            Comprehensive machine lifecycle management solution
          </p>
        </div>
      </div>
    </div>
          ...
          <div class="bg-black/20 p-6 rounded-xl border border-white/10">
            ...
            <div class="space-y-5">
              <div>
                ...
                />
                <p v-if="errors.Name" class="text-rose-500 text-xs mt-1.5 ml-1">
                  Name is required
                </p>
              </div>
...
import { useMachine } from "@/hooks/inventory/useMachine";
    ...
    cellRenderer: (params: any) => {
      return `
            <div class="flex justify-center items-center">
                <i class="${
                  params.value
                    ? "text-green-500 mdi mdi-checkbox-marked"
                    : "text-gray-500 mdi mdi-checkbox-blank-outline"
                } text-xl"></i>
            </div>
            `;
    },
...
async function saveMachine() {
  ...
  if (!machineForm.value.MachineID) {
    ...
    response = await editMachine(
      machineForm.value.MachineID,
      machineForm.value
    );
  }
...
async function handleDelete(machineId: number) {
  ...
  const confirmed = await showConfirmToast(
    `Are you sure you want to delete machine name ${machineName}?`
  );
  if (confirmed) {
