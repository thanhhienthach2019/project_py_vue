      ...
      <div class="flex items-center space-x-6">
        ...
        <div>
          ...
          <p class="text-indigo-200 mt-2 font-medium">
            Comprehensive material lifecycle management solution
          </p>
        </div>
      </div>
    </div>
    ...
    >
      ...
      <div class="grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div
            class="bg-black/20 p-6 rounded-xl border border-white/10 h-full flex flex-col"
          >
            <div class="flex-1">
              <h3
                class="text-lg font-semibold text-white mb-6 flex items-center space-x-2"
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-4">
            ...
            <div class="relative group">
              ...
              <div
                class="w-14 h-10 bg-white/10 backdrop-blur-lg rounded-xl border border-white/10 group-hover:border-blue-400/40 transition-all duration-300 flex items-center justify-center"
              >
                <svg class="w-8 h-8" viewBox="0 0 24 24" fill="none">
                  <path
                    d="M6 19L6 14M18 19L18 14"
                    stroke="currentColor"
                    stroke-width="1.5"
                    stroke-linecap="round"
                  />
                  <path
                    d="M21 14H3C3 11.7909 4.79086 10 7 10C7 6.68629 9.68629 4 13 4C16.3137 4 19 6.68629 19 10C21.2091 10 23 11.7909 23 14C23 16.2091 21.2091 18 19 18H5C2.79086 18 1 16.2091 1 14C1 11.7909 2.79086 10 5 10"
              ...
              <div
                class="absolute -bottom-1 left-4 right-4 h-1 bg-white/5 blur-sm rounded-full group-hover:bg-blue-400/30 transition-colors"
              ></div>
            </div>
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-center space-x-2 mt-1">
                ...
                >
                <span
                  class="text-sm font-semibold text-purple-300 bg-purple-400/10 px-2 py-0.5 rounded-full flex items-center"
                >
                  <span
                    class="w-2 h-2 bg-purple-400 rounded-full mr-1.5 animate-pulse"
                  ></span>
            ...
            >
              ...
              <span class="text-xs font-medium text-gray-200"
                >+{{ Math.floor(materials.length * 0.12) }}%</span
              >
...
import { useMaterial } from "@/hooks/inventory/useMaterial";
...
const { fetchMaterials, addMaterial, editMaterial, removeMaterial, materials } =
  useMaterial();
  ...
  try {
    if (materialForm.value.MaterialID === 0) {
      ...
      if (response.success) {
        ...
        toast?.value?.showToast(
          response.message || "Failed to add material. Please try again.",
          "error"
        );
      }
    } else {
      const response = await editMaterial(
        materialForm.value.MaterialID,
        materialForm.value
      );
      if (response?.success) {
        toast?.value?.showToast(
        ...
        toast?.value?.showToast(
          response?.message || "Failed to update material. Please try again.",
          "error"
        );
      }
    }
...
const handleDelete = async () => {
  if (selectedMaterialId.value !== null) {
    try {
      ...
      toast?.value?.showToast(
        "An error occurred while deleting the material. Please try again.",
        "error"
      );
    }
...
const {
  ...
  onFirstDataRendered,
  resizeNow: resizeNow,
} = useAutoResizeGrid(gridApi, gridContainer, columnsToAutoSize);
const debouncedResize = debounce(async () => {
  await nextTick();
  resizeNow();
}, 200);

watch(materialForm, debouncedResize, { deep: true });
